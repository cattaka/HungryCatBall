package net.cattaka.hungrycatball.gl;
...
public class CtkGL implements GL, GL10, GL10Ext, GL11, GL11Ext {
    ...
    private ViewportArgs viewportArgs = new ViewportArgs();
    ...
    public void calcWorldPos(float[] dstPos, float x, float y) {
        float rx = (x - viewportArgs.x) / viewportArgs.width;
        float ry = 1.0f - (y - viewportArgs.y) / viewportArgs.height;
        dstPos[0] = ((frustumfArgs.right - frustumfArgs.left) * rx + frustumfArgs.left) * 2f;
        dstPos[1] = ((frustumfArgs.top - frustumfArgs.bottom) * ry + frustumfArgs.bottom) * 2f;
        // FIXME frustumの値は半分の値が設定されてるので無理やり２倍にしている
    }

    public void calcPickRay(float[] dstPos, float x, float y) {
        // TODO
    }
    ...
    public void glViewport(int x, int y, int width, int height) {
        viewportArgs.x = x;
        viewportArgs.y = y;
        viewportArgs.width = width;
        viewportArgs.height = height;
